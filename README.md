...
- Cursor は互換読込で `.claude/skills` / `~/.claude/skills` を参照できるため、`~/.cursor/skills` への symlink は必須ではありません。
- Codex / Windsurf / Antigravity は各ツールの skills パス（例: `~/.agents/skills`）に symlink またはコピーして公開します。
