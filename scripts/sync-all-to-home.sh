...
echo ""
echo "--- skills の共通化（推奨） ---"
echo "Claude Code 用のグローバルスキルパス（~/.claude/skills/）を他ツールから参照:"
echo ""
echo "  Codex: ~/.agents/skills/"
echo "  Windsurf: ~/.codeium/windsurf/skills/"
echo "  Antigravity: ~/.gemini/antigravity/skills/"
echo ""
echo '  for d in ~/.agents ~/.codeium/windsurf ~/.gemini/antigravity; do rm -rf "$d/skills" 2>/dev/null; mkdir -p "$d"; ln -sf "$HOME/.claude/skills" "$d/skills"; done'
echo ""
