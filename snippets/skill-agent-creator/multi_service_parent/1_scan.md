...
親ディレクトリ側に既存資産が存在する場合、以下を Read して品質評価する：
...
- 自然文依頼での発動率を上げる設定（Cursor rules / Codex developer_instructions）の必要性を判定する

g. **description-first 達成度の確認**

- 静的マッピング表なしで description と Trigger Keywords のみで発動するか
- 例外的にマッピングが必要な場合は、その理由が明確に説明できるか
...
| 種別                             | 結果              | 詳細                                            |
| -------------------------------- | ----------------- | ----------------------------------------------- |
| 自然文レビュー依頼のルーティング | {OK/NG}           | {description-first で担保されているか}          |
| 静的マッピング混入（routing bloat） | {件数}         | {`.cursor/rules` / `CLAUDE.local.md` に agent 名直書きがないか} |
| Codex routing assets             | {OK/NG}           | {`developer_instructions` / preset の有無}      |
| description-first 達成度         | {LOW/MEDIUM/HIGH} | {静的マッピング不要で発動するか}                |
...
- ルート憲法（`CLAUDE.md`/`AGENTS.md`）への静的マッピング追記を避け、ルーティングは description-first で解決する
