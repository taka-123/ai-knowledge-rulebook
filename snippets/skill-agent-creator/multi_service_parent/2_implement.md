...
- `.claude/skills/` — Claude Code 用スキル（必要時）
...
- ルート憲法（`CLAUDE.md` / `AGENTS.md`）への静的マッピング追記は禁止。ルーティングは description-first で skill/agent の description と Trigger Keywords によるマッチングで実現する
...
5. ルーティング資産を更新（必要時: `.cursor/rules/`, `.cursor/commands/`, `BUGBOT.md`, `.codex/REVIEW_PLAYBOOK.md`）
