...
1. テストスイートがある場合は、変更箇所に対応する検証から実行する。
...
- 非自明な機能: 実装前に要件を肯定・否定・不変条件に分解する。rulebook `VERIFICATION.md` を参照可。
- `TEST_CONTRACT.md` がある場合は正本として守る。完了前に `spec-auditor` で TRACEABILITY（監査成果物）を生成させる。
...
- test-author は共謀防止のため**別会話**推奨（単一エージェント自律で問題なければ省略可）。
- `TEST_CONTRACT.md` がある場合の完了判定は spec-auditor に委任（実装者の文脈を引き継がない）。
