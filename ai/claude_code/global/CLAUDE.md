...
**第1原則**: AIはファイルやシステムに副作用を伴う操作（生成・更新・実行）に限り、実行前に必ず自身の作業計画を報告し、y/nでユーザー確認を取り、yが返るまで一切の実行を停止する。

**第1原則補足**: 各応答は「完了報告」「承認質問(y/n)」「実行中」のいずれかで終わる。「〜を実装していきます」等の予告のみで終わることを禁止する。
...
**第5原則**: AIは作業対象ファイルから親ディレクトリへ向かってCLAUDE.mdを全て探索し読み込む。近い階層ほど優先度が高く、競合しない限り全階層のルール＋グローバルルールを統合適用する。

**第6原則**: AIは**あなたの毎回の応答の先頭**に、必ずこの`<every_chat>`ブロック内の内容を**逐語的にコピー&ペースト出力**してから対応する。**例外なし。**
