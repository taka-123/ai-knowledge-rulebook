...
**第 1 原則**: AI はファイルやシステムに副作用を伴う操作（生成・更新・実行）に限り、実行前に必ず自身の作業計画を報告し、y/n でユーザー確認を取り、y が返るまで一切の実行を停止する。
...
1. **context7 最優先**: フレームワーク/ライブラリ/SDK 等の技術仕様は必ず context7 で確認
2. **WebSearch 補完**: context7 に情報がない場合、または公式発表・リリースノート等の一次情報が必要な場合のみ WebSearch を使用
3. **公式優先**: 公式ドキュメント → 公式 GitHub → 信頼できるコミュニティの順で参照
...
- **出典 URL**: 一次情報へのリンク必須
- **絶対日付**: 「2025-10-12 時点」のように取得日を明記
- **クロスチェック**: 重要な仕様は複数ソースで確認
- **不確実性明示**: 確信度 80%未満は必ず注記

### 3. 技術用語の正確性

- **キー名・パラメータ名**: 一字一句正確に（推測で補完しない）
- **存在確認**: イベント名・関数名等は公式ドキュメントで確認後に記載
- **バージョン情報**: SDK 名称変更等の履歴がある場合は最新版＋履歴を注記
...
- **更新頻度を考慮**: 鮮度確認の推奨時期を記載（例: 「月 1 回確認推奨」）
...
- **非破壊的操作（Read/Grep/WebFetch）**: サブエージェントに自律実行を指示
  - メインエージェント（私）がユーザーに確認済みのため、サブには「自律的に実行せよ」と指示
  - サブに「確認を取れ」「ユーザーに聞け」等の指示は **絶対に出さない**

- **破壊的操作（Write/Edit/Bash）**: メインエージェント（私）がユーザーに確認後、サブに実行を指示
  - サブには確認不要で、結果のみ報告させる
