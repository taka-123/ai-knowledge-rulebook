...
| --- | --- | --- | --- |
| **Fast draft / Prototype** | Gemini | `gemini "$ARGUMENTS"` | 高速ドラフト、プロトタイプ作成 |
| **Breakdown / Planning** | Gemini | `gemini "$ARGUMENTS"` | 初動仕様分解、タスクブレイクダウン |
| **Bulk processing** | Gemini | `gemini "$ARGUMENTS"` | 大量ファイル整理、反復作業 |
| **Research / Search** | Gemini | `gemini "$ARGUMENTS"` | 最新情報スキャン、市場調査、Web検索 |
| **UI / Visual** | Cursor | `cursor-agent -p "$ARGUMENTS"` | UI調整、レスポンシブ最適化 |
| **Visual diff** | Cursor | `cursor-agent -p "$ARGUMENTS"` | 目視diff確認、微調整 |
| **Doc + Code polish** | Cursor | `cursor-agent -p "$ARGUMENTS"` | コード＋ドキュメント同時洗練 |
| **Exploratory prototype** | Cursor | `cursor-agent -p "$ARGUMENTS"` | 探索的プロトタイピング |
| **Parallel implementation** | Codex | `codex "$ARGUMENTS"` | 複数機能同時並列実装 |
| **Deep debug / Optimize** | Codex | `codex "$ARGUMENTS"` | 低レベルdebug、アルゴリズム最適化 |
| **Long-running task** | Codex | `codex "$ARGUMENTS"` | 夜間・長時間放置タスク（30分以上OK） |
| **Spec / PRD / Test plan** | Codex | `codex "$ARGUMENTS"` | 厳密な構造文書・PRD・テスト計画 |
...
Output: Gemini を選択（高速ドラフト向き）、`gemini "React コンポーネントの初期設計をドラフトして"` を実行、結果を要約。
...
Output: Cursor を選択（UI/Visual 向き）、`cursor-agent -p "..."` を実行。
