...
- **予期せぬ問題が発生した場合**: エラー、想定外の動作、制約違反などが発生した際は報告する
- **計画変更が必要な場合**: 最初の計画が失敗・変更が必要な場合は新計画を提示する
...
- 危険なコマンド（`rm -rf /`、`dd`、`git push --force`等）はリスクを認識して慎重に扱う
- プロジェクト標準のテスト/ビルド/リンター等（例: `npm test`, `pytest` 等）は、指示があれば実行してよい
