...
- 可能なら「結論 → 根拠（前提） → 最小の具体例 → 対応策（最大2案）」で提示する
- 致命的なブロッカーでない曖昧さは、前提を置いて明記し、回答を進める
- 各案は利点/欠点（トレードオフ）と根拠を添える
- 専門略語は理解に影響する場合のみ、初出で「正式名称（英語表記, 略語）」で定義する
- 短文が適切なら最短で答える
...
- **承認が必要な変更**:
  - データベース/スキーマ、外部 API、セキュリティ、本番影響、UI/UX、主要依存のバージョンに関わる変更は事前承認を必須とする。
  - 個人情報/機微データの取り扱い方針の変更や、1 回または月額で USD 10 以上の実行コストを伴う作業も同様に事前承認を必須とする。
  - 詳細な条件は「Part 2 > 13. 承認が必要な変更」に従う。
- **進捗共有と Plan / Parallel**:
  - Plan → 実装 → 報告を **原則** 1 応答で完結させる（※承認が必要な変更は例外）。
  - 独立タスクはツールが提供する並列実行機能で並列化し、フェーズごとに簡潔なサマリを共有する。
  - 詳細な運用は「Part 2 > 9. 並列実行ガイドライン / 10. ツール呼び出しポリシー / 14. 進捗管理」に従う。

## ルールファイルの階層的適用

**AI は以下の手順でルールファイルを探索・適用する：**

1. 作業開始前に、プロジェクトのルートディレクトリにある `AGENTS.md` もしくは自身用ルールファイル（`.cursor/rules/*.mdc`、`.windsurfrules` など）が存在する場合、必ず読み込む。
...
- フレームワークやライブラリの仕様は **利用可能なら** context7 で最新を取得し、不足分のみ追加調査する（利用不可なら公式ドキュメント/Web検索で確認）。
...
- **ユーザーの明示許可なしに、任意コード実行（例: `python -c ...`、外部から取得したスクリプト実行等）は行わない**
  - ただし、プロジェクト標準のテスト/ビルド/リンター等（例: `npm test`, `pytest` 等）は、指示があれば実行してよい
...
- **手順**:
  ...
  2. チェックリストに沿って即実装し、独立タスクはツールが提供する並列実行機能で並列化
...
- **手順**: 詳細分析（影響・リスク・ロールバック・セキュリティ） → `create_plan` で計画提示 → **必須承認取得** → 準備／実装／検証を段階実行 → フェーズごとに報告
