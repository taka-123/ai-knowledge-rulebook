...
**モノレポ/マルチサービス構成のプロジェクトでは、すべてのファイル編集前に確認:**

- [ ] 作業対象ファイルのパスを特定する
- [ ] サービス/モジュール配下に `AGENTS.md` が存在するか確認
- [ ] **存在する場合 → 該当ディレクトリの `AGENTS.md` を読み込む**
- [ ] 読み込み完了後、その内容を最優先ルールとして適用

**一般的なパターン:**
- サービスディレクトリ直下の `AGENTS.md` を確認
- プロジェクトルート `AGENTS.md` でルールを確認
...
- **CLI例外**: ファイル内容確認・ディレクトリ構造表示等の読み取り専用操作は、分析・報告なしで即座に実行し、結果のみを1〜2行で回答してよい
...
- **手順**:
  ...
  3. 実装（計画と同一メッセージ内で進行可能）
  4. 各ステップ完了後に簡潔な進捗報告
  5. 検証（テスト・レビュー）→ 最終報告

**原則**: 🟡標準タスクは「計画提示 → 実装 → 報告」を1つの応答で完結させ、効率的に進める
