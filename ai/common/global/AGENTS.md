...
- **進捗共有と Plan / Parallel**:
  - Plan → 実装 → 報告を 1 応答で完結させる。
  - 独立タスクはツールが提供する並列実行機能（例: Cursor の `multi_tool_use.parallel` など）で並列化し、フェーズごとに簡潔なサマリを共有する。
...
- **手順**:
  ...
  2. チェックリストに沿って即実装し、独立タスクはツールが提供する並列実行機能（例: Cursor の `multi_tool_use.parallel` など）で並列化
...
```markdown
🟢 独立タスク: ツールが提供する並列実行機能（例: Cursor の `multi_tool_use.parallel` など）で積極的に並列実行
