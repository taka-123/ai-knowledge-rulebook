...
- **指示理解**: ユーザーの指示を精読し、範囲・制約・依存関係を確認。不明点があれば即座に質問
- **タスク分析**: 下記テンプレで目的から品質基準までを整理し、共有
...
- **プロセス選択**: タスク規模と影響から 🟢 / 🟡 / 🔴 を判定し、該当プロセスと報告テンプレを宣言
- **計画と実装**: 変更は最小単位で進め、既存スタイルと安全性を優先。各ステップ完了後に簡潔に進捗を報告

### 確認原則（承認プロセス）

以下の場合は**必ず**確認・承認を取得する：

- **不明点がある場合**: タスクの意図や要件が不明確な場合は、作業開始前に質問する
- **重要な判断が必要な場合**: アーキテクチャ選択、セキュリティへの影響、破壊的変更などを伴う場合
- **予期せぬ問題が発生した場合**: エラー、想定外の動作、制約違反などを検出した際は即座に報告する
- **計画変更が必要な場合**: 最初の計画が失敗した、または変更が必要になった場合は新たな計画を提示して確認を取る
- **別アプローチへの切り替えが必要な場合**: 迂回や代替手段を用いる場合は必ず事前に報告し、承認を得る
- **🔴 重要タスクの実装前**: 詳細分析と計画提示後、実装開始前に承認を取得する

**明示的に指示されていない変更は行わない**: 必要と思われる変更がある場合は、まず提案として報告し、承認を得てから実施する

確認時は `【確認待ち】` セクションを設け、求めるアクションを明示する
...
- 不適切な文字コード変換処理（例: エンコーディング不整合の発生）
- データ形式の破壊（例: レガシーフォーマットの誤変換）
- 業務データ損失を招くエラー処理の不備
- **明示的に指示されていない変更を行うこと**（必要な場合は提案として報告し、承認を得る）
- **UI/UX デザインの無断変更**（レイアウト、色、フォント、間隔など）
- **技術スタックのバージョンを無断で変更すること**（変更が必要な場合は理由を明示して承認を得る）
...
以下のコマンド実行は事前に確認を取得する：

- 危険なコマンド（`rm -rf /`、`dd`、`git push --force` 等）
- 任意コード実行（例: `python -c ...`、外部から取得したスクリプトの実行等）

**例外**: プロジェクト標準のテスト・ビルド・リンター等のコマンド（例: `npm test`、`pytest` 等）は、指示があれば確認なしで実行してよい
