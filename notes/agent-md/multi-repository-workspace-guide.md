...
**最終更新**: 2025-10-17
**ステータス**: 公式仕様・コミュニティ情報検証済み、実務投入可能
...
> "Project-specific rules: Place rules in `.cursor/rules/*.md` in your project root"

**重要**: プロジェクトルート（起動ディレクトリ）基準で解釈される

#### サブディレクトリルール

Cursorでは、サブディレクトリごとに `.cursor/rules/*.md` を配置することで、ディレクトリごとに異なるルールを適用可能。ただし、親ディレクトリから起動した場合、子ディレクトリのルールは自動適用されない。
...
**A**: Claude CodeとWindsurfは**AGENTS.mdをネイティブサポートしていません**。使用する場合は明示的な参照が必要です。
...
```
プロジェクト/
├── AGENTS.md .................... Codex/Cursor用（ネイティブ）
├── CLAUDE.md .................... Claude Code用（AGENTS.md参照を含む）
├── .windsurfrules ............... Windsurf用（AGENTS.md参照を含む）
└── .cursor/rules/*.md ........... Cursor用追加ルール（オプション）
```

**重要**: Claude CodeとWindsurfでは、AGENTS.mdを「ルールファイル」としてではなく、「参照すべきドキュメント」として扱う必要があります。
